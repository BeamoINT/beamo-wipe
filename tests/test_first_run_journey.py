...
def test_blocked_error_keeps_support_detail_out_of_owner_ui(monkeypatch, tmp_path):
    """Journey guidance still records support detail; main keeps it off the kiosk."""
    monkeypatch.setattr("beamo_wipe.safety.default_log_dir", lambda: tmp_path)
    ...
    assert wiz.error == C.IDENTIFY_ERROR
    assert "TimeoutExpired" not in (wiz.error or "")
    assert "Support detail" not in (wiz.error or "")
    diag = (tmp_path / "diagnostics.log").read_text(encoding="utf-8")
    assert "pick_blocked" in diag
