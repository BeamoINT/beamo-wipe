            ...
            if phrase == "nwipe sanitize" and "vendor" in blob:
                ...
                for line in blob.splitlines():
                    if "nwipe sanitize" in line and "vendor" not in line:
                        assert False, (
                            f"forbidden claim in surface: {phrase!r} line: {line[:120]}"
                        )
                continue
...
def test_evidence_warning_matches_ssd_footer_language():
    ...
    disc = DiscoveryResult(
        disks=(disk,), selectable=(disk,), boot=None, boot_identified=True
    )
    ev = build_evidence(
        disk=disk,
        discovery=disc,
...
def test_package_readme_inside_iso_says_no_warranty_and_nwipe():
    # The staged README.txt is generated by the live asset stager.
    assets = _lower("scripts/stage_live_assets.py")
    assert "nwipe" in assets
    assert "no warranty" in assets
...
def test_active_support_docs_match_canonical_methods_and_outcomes():
    ...
    for path in (
        "docs/screens.md",
        "docs/runbook.md",
        "docs/storage-and-controller-limits.md",
    ):
        text = _lower(path)
        for obsolete in (
            "not as thorough",
            ...
            "then optionally beamo for an extra pass",
        ):
            assert obsolete not in text, (path, obsolete)
    assert "A reported write failure never becomes successful completion" in _read(
        "docs/storage-and-controller-limits.md"
    )
