...
def test_live_config_bootstrap_uses_https_debian_mirrors():
    bootstrap = ROOT / "packaging/live/config/bootstrap"
    binary = ROOT / "packaging/live/config/binary"
    if not bootstrap.is_file() or not binary.is_file():
        import pytest

        pytest.skip("lb config artifacts are generated by scripts/build-iso.sh")
    bootstrap_text = bootstrap.read_text(encoding="utf-8")
    assert "https://deb.debian.org/debian/" in bootstrap_text
    assert "https://security.debian.org/" in bootstrap_text
    assert "http://deb.debian.org/" not in bootstrap_text
    assert "http://security.debian.org/" not in bootstrap_text
    binary_text = binary.read_text(encoding="utf-8")
    assert "ip=frommedia" in binary_text
    assert 'LB_FIRMWARE_BINARY="false"' in binary_text
    assert 'LB_FIRMWARE_CHROOT="false"' in binary_text
    failsafe_cfg = [
        line
