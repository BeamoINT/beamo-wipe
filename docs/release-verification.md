...
| --- | --- | --- | --- |
| `beamo-wipe-0.2.4-amd64.iso` | Bootable live image (hybrid BIOS+UEFI) | Operator-defined after authorization | local `dist/` until separately published |
| `beamo-wipe-0.2.4-amd64.iso.sha256` | SHA256 sidecar (`<sha>  <name>`) | same | `dist/` alongside ISO |
| `beamo-wipe-0.2.4-amd64.manifest.json` | Release provenance (this doc) | same | `dist/` |
| `beamo-wipe-0.2.4-amd64.manifest.json.sha256` | Manifest checksum | same | `dist/` |
...
sha256sum -c beamo-wipe-0.2.4-amd64.iso.sha256
sha256sum -c beamo-wipe-0.2.4-amd64.manifest.json.sha256
# From repo root, change directory because each sidecar intentionally binds a
# bare filename rather than an arbitrary path:
(cd dist && sha256sum -c beamo-wipe-0.2.4-amd64.iso.sha256)
(cd dist && sha256sum -c beamo-wipe-0.2.4-amd64.manifest.json.sha256)
(cd dist && sha256sum -c SHA256SUMS)

# From a checked-out v0.2.4 source tree, place the downloaded release files
...
verify_manifest(pathlib.Path("dist/beamo-wipe-0.2.4-amd64.manifest.json"))
print("manifest OK")
PY

# Inspect provenance without trusting ISO:
python3 -m json.tool dist/beamo-wipe-0.2.4-amd64.manifest.json | head -n 60
