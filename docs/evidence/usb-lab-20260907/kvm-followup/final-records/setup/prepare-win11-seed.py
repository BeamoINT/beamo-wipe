...
for name in ('start-trace.ps1','stop-trace.ps1'):
 shutil.copy('/lab/setup/'+name,root/name)
image=root/'seed.raw'
with image.open('xb') as f:f.truncate(64*1024**2)
subprocess.run(['mkfs.vfat','-F','32','-n','USBLAB_SEED',str(image)],check=True)
for name in ('Autounattend.xml','bootstrap.ps1','windows_probe.ps1','diagnostics.ps1','start-trace.ps1','stop-trace.ps1'):
