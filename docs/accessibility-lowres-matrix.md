...
| 2 | **OWNER** | Owner checkbox card (takefocus=1, ring) | Card → `Back` → `Choose a disk` → wrap | Card `ring=True` → `FOCUS` outer when focused; checked halo `PRIMARY_TINT` | `Space` toggles card; `Enter` only when `owner_ok`; `Esc` → What | Tab cycles 3, card Space does not trap; held Space guard `arm_done_keyboard` | pass | pass (card `WRAP-140`) | Checkbox unchecked `BORDER_STRONG`, checked `PRIMARY` + halo | `Esc`/`Back` → What | Checked icon + halo + text, not color alone |
