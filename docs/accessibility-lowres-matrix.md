...
| 1 | **OWNER** | Owner checkbox card (takefocus=1, ring) after explanation bullets | Card → `Back` → `Continue` → wrap; `Show more` in body | Card `ring=True` → `FOCUS` outer when focused; checked halo `PRIMARY_TINT` | Read bullets and power reminder; `Space` toggles card; `Enter` only when `owner_ok`; `Esc` → Keyboard | Tab cycles card/Back/Continue plus Show more; held Space does not trap | pass | degraded (combined bullets + checkbox; body scrolls on short windows) | Info panel `Secure Boot hint + nwipe line` with info badge; checkbox unchecked `BORDER_STRONG`, checked `PRIMARY` + halo | `Esc`/`Back` → Keyboard | Bullet amber dots + checkbox icon + halo + text, not color alone |
...
| 5 | **METHOD** | `Continue` (primary right) | Assist utilities (`Advanced`, `Check disks again`, `Need a report?`) → `Back` → `Continue`; cards are not Tab-focusable, selection via `1/2/3` or `Up/Down` | Footer buttons ring; selected card `halo` + `PRIMARY` border; hover `SURFACE_ALT` | `1/2/3` selects Everyday/Extra/Quick zero; `Up/Down` cycles order; `Enter` → Last chance | Tab cycles assist then Back/Continue, card Up/Down never traps; `compact` title block keeps height inside 740 | pass | pass (compact margins `top 10/bot 6`, center_zone) | Each card `blurb` + `pace` with clock icon; assist-strip hint `1, 2, or 3` as key caps | `Esc`/`Back` → Confirm; `Advanced` → Advanced | Radio selected vs unselected + Recommended chip, not color alone |
