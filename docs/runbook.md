...
> **Version 1.6 — 2026-09-13 | Owner: Accountable senior engineer (this checkout) | Next review 2026-12-13**
> Pinned wrapper `0.2.8` / `nwipe v0.42` commit `6082bde060091e66365d852a1877f2ee80c67105` at `/usr/lib/beamo-wipe/nwipe`
...
| Environment | Wrapper version (`src/beamo_wipe/__init__.py 0.2.8`), live `NWIPE_VERSION` on USB, firmware mode (BIOS vs UEFI, Secure Boot on/off), machine vendor/model, bus of target (`TRAN`), kind (`ROTA`→HDD vs SSD per `classify_kind`) | Strip customer name | Needed for §3 wear/raid decision |
...
Symptom: stick never appears (Dell F12, HP F9/Esc, Lenovo F12 not listed)
  ...
  │   │       Reflash on Linux: `sudo dd if=dist/beamo-wipe-0.2.8-amd64.iso ...` (verify `/dev/sdX` *is* USB via `lsblk`), or BalenaEtcher. Retry.
...
BEAMO_WIPE_VERSION=0.2.8 ./scripts/qemu-verify.sh
...
- Manifest `pinned nwipe commit 6082bde060091e66365d852a1877f2ee80c67105` or `__version__` drift from `src/beamo_wipe/__init__.py` (currently `0.2.8`; check `python -m pytest tests/test_live_image.py::test_staged_chroot_package_matches_src`).
