...
> Pinned wrapper `0.1.1` / `nwipe v0.42` commit `6082bde060091e66365d852a1877f2ee80c67105` at `/usr/lib/beamo-wipe/nwipe`
...
| Environment | Wrapper version (`src/beamo_wipe/__init__.py 0.1.1`), live `NWIPE_VERSION` on USB, firmware mode (BIOS vs UEFI, Secure Boot on/off), machine vendor/model, bus of target (`TRAN`), kind (`ROTA`→HDD vs SSD per `classify_kind`) | Strip customer name | Needed for §3 wear/raid decision |
...
Symptom: stick never appears (Dell F12, HP F9/Esc, Lenovo F12 not listed)
  ...
  │   │       Reflash on Linux: `sudo dd if=dist/beamo-wipe-0.1.1-amd64.iso ...` (verify `/dev/sdX` *is* USB via `lsblk`), or BalenaEtcher. Retry.
...
# BIOS:
qemu-system-x86_64 -m 2048 -enable-kvm -cdrom dist/beamo-wipe-0.1.1-amd64.iso \
