...
if [[ "$($GO_BIN version | awk '{print $3}')" != go1.26.8 ]]; then
  echo "Launcher builds require Go 1.26.8 (see scripts/ci-desktop.sh)." >&2
  exit 2
fi
...
cd "$ROOT/desktop"
FLAGS="-s -w -X main.version=$VERSION -X main.sourceCommit=$SOURCE -X main.sourceDirty=$DIRTY"
...
(root/'desktop-build.json').write_text(json.dumps({'version':sys.argv[2],'source_commit':sys.argv[3],'go':'go1.26.8','files':files},sort_keys=True,indent=2)+'\n')
PY
