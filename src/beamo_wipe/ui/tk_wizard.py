...
class TkWizard:
    ...
    def _prepare_body_host(self) -> None:
        ...
        if self._body is None or not self.lay.short:
            return
        if self.w.screen in {
            Screen.PICK,
            Screen.LIMITS,
            ...
            return
        canvas = tk.Canvas(self._body, bg=BG, highlightthickness=0, bd=0)
    ...
    def _wrapping_label(self, parent: tk.Widget, text: str, *, font, bg: str,
        ...
        label = tk.Label(parent, text=text, font=font, fg=fg, bg=bg,
                         anchor="w", justify=tk.LEFT, wraplength=max(200, self.lay.wrap - 120))
        def fit(event) -> None:
    ...
    def _status_screen(self, kind: str, title: str, message: str) -> None:
        ...
        self._p(
            ...
            wraplength=min(700, self.lay.wrap), justify=tk.CENTER, anchor="center",
        ).pack(fill=tk.X)
    ...
    def _limits(self) -> None:
        ...
        text.configure(state=tk.DISABLED)
        text.focus_set()
        row = self._footer_shell("Esc returns to method selection.")
        self._back_btn(row)
